    ...
    while(True):
        ...
        while(True):
            ...
            if key == ord("q"):
                ...
                return np.array(eef_positions_in_robot_frame), np.array(eef_rotmats_in_robot_frame), np.array(marker_positions_in_camera_frame), np.array(marker_rotmats_in_camera_frame)
            if key != -1:
                robot_control.endTeachMode()
...
if __name__ == "__main__":
    ...
    eef_pos_in_robot, eef_rotmat_in_robot, marker_pos_in_cam, marker_rot_matrix_in_camera = collect_eye_to_hand_data(cv2.aruco.DICT_6X6_250,0.103)
    ...
    if mode == "eye-in-hand":
        camera_rotmat_in_gripper, camera_pos_in_gripper = cv2.calibrateHandEye(eef_rotmat_in_robot, eef_pos_in_robot,marker_rot_matrix_in_camera, marker_pos_in_cam)
    if mode == "eye-to-hand":
        ...
        camera_rotmat_in_base,camera_pos_in_base = cv2.calibrateHandEye(robot_rotmat_in_eef,robot_pos_in_eef,marker_rot_matrix_in_camera, marker_pos_in_cam)
